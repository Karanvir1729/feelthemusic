...
5. **Everything is scheduled on the shared clock, never on packet arrival.** The conductor stamps each event with a presentation time that **already includes** the fixed room latency budget `L` (300 ms); every client fires it at that time converted to its own clock, minus that client's own output latency, and never adds `L` again (measured on the real conductor: `masterTs` minus arrival matched `leadUs`, about 267 ms, on a shared clock; adding `L` a second time fires about 300 ms late). Audio anchors carry a presentation time that needs its own latency conversion. Do not try to minimise `L`.
