...
the head twice as far again and still pass the safety envelope). It is measured on beat_clips itself
(4 tiers x 3 variants, beat_clips.commanded -> delivered) over every tempo BeatTracker can lock (80-214
bpm) and every `bold`: the boldest delivered yaw excursion is 52.0 units (hype a, the calibrated sweep, at
80 bpm) and the deepest lift |d elbow_pitch - d base_pitch| is 24.2 (the build's crouch, also at 80 bpm).
X_RANGE_UNITS / Y_RANGE_UNITS sit 3-4 % above those. With FK the same yaw excursion and the reference
poses (Y_UP_REF = the drop's spring, Y_DOWN_REF = the build's crouch) are pushed through the model, and y
is widened by Y_HEADROOM because the real crouch goes past the reference pose; the FK path also picks up
the sine flattening of a wide sway, which the joint-unit path cannot see (below).

  re-measured 2026-09-19 against the calibrated presets (PR #34) at x 54 / y 25, dt 50, bold 1.0:
      tier    |x| p50 / max     |y| p50 / max     radial p50 / max    points pinned at the edge
      groove  0.25 / 0.52       0.32 / 0.42       0.44 / 0.61        0 / 2865
      hype    0.40 / 0.96       0.72 / 0.88       0.92 / 1.20        0 / 2961
      drop    0.34 / 0.82       0.66 / 0.94       0.71 / 1.09        0 / 2828
      build   0.13 / 0.26       0.97 / 0.97       0.97 / 1.00        0 / 2884
  A typical clip fills 0.73 of the range (radial p50 over all twelve, 0.66 at DEFAULT_BOLD 0.6) and the
  boldest preset at the boldest setting reaches 0.96 of x and 0.97 of y without a single clipped point,
  at any tempo or bold. Only x changed: PR #34 leaves the vertical range exactly where it was (24.2 units,
  still the build's crouch) and makes the sweep 1.85 x wider sideways, so X_RANGE_UNITS went 29 -> 54 and
  Y_RANGE_UNITS did not move. At the old 29 the new hype tier pinned 371 points to the edge (37 % of the
  worst clip) -- the sweep, the biggest move in the show, would have drawn a flat line along the side of
  the screen. Keeping the scale honest costs the groove tier apparent width (its yaw did not grow, so it
  is now 0.44 of the range instead of 0.58); that is the price of one frame in which the sweep really is
  1.85 x the groove, and it costs the scorer nothing: the share of a clip under its 0.08 units/s floor
  moves by at most 2 points and every clip still offers 1.7-2.5 s of ball movement in its best 2.5 s
  window, against the 1.0 s the phone needs to stay `act`ive.

  CENTIMETRES, calibrated against @tonycodex's model figures for PR #34 (16.7 cm of lateral sweep and
  10.5 cm of rise at 132 bpm, bold 1.0): x 1.0 ~ 10.2 cm of head travel, y 1.0 ~ 8.3 cm. The ball's box is
  20.4 x 16.6 cm of real head movement where it was 13.3 x 14.7, i.e. wider than it is tall now where it
  used to be slightly taller than wide: #34's dance is a sideways dance. The lateral figure is amplitude-dependent --
  travel goes as sin(yaw), so a unit of yaw is worth 0.188 cm across the full sweep and 0.216 cm across
  the old small one -- which is why these are quoted at the sweep's own size and why the FK path, where
  there is a vendor description, is the better of the two.
  `python3 follow_game.py <clip> --fk` prints the ranges in metres where there is a vendor description.

PR #34's GENERATOR, and what this file depends on. beat_clips grew named presets (DANCE_PATTERNS:
auto / sweep / rise / diagonal / wiggle, pinning hype a/b/c and build b) and moved the `bold` multiplier to
AFTER the per-joint speed budget, so a speed-limited joint still answers the slider. Nothing this file
reads changed: START, GAINS, FPS, HOLD_S, BEATS, TIERS, VARIANTS, the CSV header and commanded()'s (N, 5)
frame order are identical on main, on PR #32 and on PR #34, and the lamp-frame signs are unchanged (the
sign-bearing constants Y, ACCENT, FLICK, UP, DOWN, FOLD and MIRROR are byte-identical across all three;
hype a still leads to the lamp's own right, and every new pose still lifts the head with elbow_pitch above
base_pitch). Only the sizes moved. Two consequences worth knowing:
  * The scheduler now refuses a library clip whenever bold < 1 (ClipScheduler.pick returns None) and will
    only post a live clip that matches the current preset, bold and tempo. So a beat_<tier>_<bpm>_<v> name
    that reaches us is always bold 1.0 -- which is exactly what generate_clip() regenerates -- and a live
    clip's real amplitude comes from its own CSV in the pack dir. Neither needs the slider's value here.
  * When nothing matches, the scheduler posts nothing at all for those 8 beats. That is not a gap to paper
    over: the arm really is still, the rest path covers a few seconds of it and then the ball leaves the
    screen, which is what the contract asks for.
...
X_RANGE_UNITS = 54.0                                   # delivered base_yaw units = x 1.0 (the calibrated sweep reaches 52.0)
...
def record(out_path: str, tempos, tiers, variants, pub: "PathPublisher") -> dict:
    ...
    doc = {"v": 1, "made": time.strftime("%Y-%m-%d"), "dt": pub.dt_ms, "hold_ms": int(pub.hold_ns / 1e6),
           ...
           # what 1000 means, so a reader can say how big the dance is without re-deriving it (see RANGES)
           "range": {"x_units": X_RANGE_UNITS, "y_units": Y_RANGE_UNITS, "x_cm": 10.2, "y_cm": 8.3,
                     "fk": pub.fk is not None},
