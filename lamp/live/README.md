...
- Where both touched the same logic (`LiveFollower._feed_guard` / `_send`): one step is in flight at a time and it is judged `land_settle` after the runtime accepted it (`max(sent, completed) + 0.65 s + duration`), one landing allowance, not two stacked. Two tests were rewritten against the allowance instead of the old 0.30 s number, and the byte-for-byte library check skips the re-choreographed hype/drop `b`/`c` clips when the library on disk predates them.
