...
views = {
    ...
    "puck_render_closed.png": (
        1.0,
        0.0,
        ("0.10 -0.09 0.10", "0.68 0.73 0 -0.38 0.35 0.86"),
    ),
    "puck_render_top.png": (0.0, 0.2, ("0.024 0.019 0.20", "1 0 0 0 1 0")),
}
for name, (alpha, lift, cam) in views.items():
    m = mujoco.MjModel.from_xml_string(scene(alpha, lift, cam))
    d = mujoco.MjData(m)
    mujoco.mj_forward(m, d)
    r = mujoco.Renderer(m, 800, 1100)
    r.update_scene(d, camera="c")
    png(f"renders/{name}", r.render())
    r.close()
print("rendered")
