...
The lamp generates its own choreography; the conductor only says **which** dance and **how big**. That lives in the
Control message's existing `"lamp"` object (type 13, sent to every peer), beside the mode and the light show, and it
is the authoritative definition of the two new keys:

```json
"lamp": {"mode": "dance", "lights": true, "preset": "sweep", "bold": 0.75, "gen": 12}
```

- `preset`: a string, one of `"auto"`, `"sweep"`, `"rise"`, `"diagonal"`, `"wiggle"`. Default `"auto"`, which means
  the lamp picks for itself — what it did before this key existed. The other four name the generator's patterns:
  side to side, up and down, corner to corner, small and quick.
- `bold`: a number 0…1, default 0.5 — how big the moves are ("Bigger moves" on the dashboard). 0 is the smallest the
  generator offers, 1 the largest. It is a request, not a licence: **the lamp still applies its own speed, envelope,
  geometry and ZMP limits**, and a `bold` it cannot honour is clamped there, never obeyed past a limit.
- `gen`: the counter that was already in this object. Every change to any of its keys — mode, lights, preset, bold —
  bumps it by one, so the lamp can drop clips it had queued for the old settings.

Both new keys are additive, and both sides degrade quietly:

- An **old lamp** that does not know them ignores them, as it ignores every unknown key, and keeps dancing.
- An **old conductor** never sends them; a new lamp must then stay on its own defaults (`auto`, 0.5) and must not
  read a missing key as `""` or `0`.
- The **lamp validates and may refuse**: it checks `preset` against the names it really implements (an unknown one
  falls back to `auto`) and `bold` against its limits. A refusal is not an error, and the conductor does not retry
  it — it shows what the lamp reports instead (below).

The conductor sends a Control the moment any of this changes (it does not wait for the 2 s tick) and writes the
change to the run log as `{"t":"set","lamp":{…}}`. The conductor's own validation is the same list of five names and
the same 0…1 clamp, so a lamp never sees a name or a number from outside them.

### The lamp reports back

Because the lamp may refuse, the dashboard shows what the lamp says it is **really** doing, not only what was asked
for. The lamp's once-a-second `{"t":"lamp"}` status may carry two optional keys beside the ones it already sends:

```json
{"t": "lamp", "state": "dancing", "mode": "dance", "preset": "sweep", "bold": 0.75, "locked": false, "piC": 47.5, "sdk": "ok", "moves": 12, "refused": 0}
```

`preset` is the name of the dance it is actually generating, `bold` the size it actually used, 0…1. Both are
optional: when a lamp does not send them the dashboard simply shows no dance for it and nothing else changes. The
conductor never "corrects" the lamp from these — it only displays them, in orange when they differ from what the
dashboard asked for.
